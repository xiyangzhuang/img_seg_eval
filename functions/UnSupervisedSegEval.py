...
outPolygonFeature = curDir + '\\auxiliary_files.gdb\\featurePolygon'
...
def rand_sample(sample_rate = 1, class_name = ['tree', 'building', 'water', 'car', 'other']):
    numDIR = 50
    ...
    return dictReferPolyAttr, dictReferPointAttr

def unsuperviesed_segment_evaluation(feature_name, itersect_area, dictReferPolyAttr, dictReferPointAttr):
    arcpy.PolygonToLine_management(featureName, outPolygonFeature)
    #  segmentation index 
    ASEI = 0
    ...
    for row in cursor:
        ...
        arcpy.MakeFeatureLayer_management (outPolygonFeature, "FeaturePolyline")
